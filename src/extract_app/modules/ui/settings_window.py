...
# File: src/extract_app/modules/ui/settings_window.py
# Version: 2.0.0
# Description: Settings popup window with Hybrid Translation configuration.
# --------------------------------------------------------------------------------
...
class SettingsWindow(ctk.CTkToplevel):
    """
    Modal settings window with tabbed interface (Dark Navy theme).
    Includes: Hybrid API Settings (Cloud/Local), Appearance.
    """
    
    def __init__(self, master, settings_manager, translation_service, on_close: Optional[Callable] = None):
    ...
    def _build_translation_tab(self, parent):
        ...
        self.api_key_entry = ctk.CTkEntry(
            key_frame, textvariable=self.api_key_var, show="•", width=250,
        ...
        ctk.CTkLabel(config_frame, text="(-1 = All)", text_color=Colors.TEXT_MUTED, font=Fonts.TINY).pack(side="left")
        
        # --- 4. Common Settings ---
        ctk.CTkLabel(
            scroll, text="Cài đặt chung", 
            font=Fonts.H3, text_color=Colors.TEXT_PRIMARY
        ...
        ctk.CTkButton(
            action_frame, text="Test Kết nối/Load Model", command=self._test_connection, width=180, height=40,
    ...
    def _save_settings(self):
        ...
        try:
            self.settings_manager.set("chunk_size", int(self.chunk_size_var.get()))
        except: pass
        try:
            self.settings_manager.set("chunk_delay", float(self.delay_var.get()))
        except: pass
        ...
        try:
            self.settings_manager.set("n_gpu_layers", int(self.gpu_layers_var.get()))
        except: pass
        
        self.status_label.configure(text="✓ Đã lưu cài đặt!", text_color=Colors.SUCCESS)
        ...
        if engine == "cloud":
            try:
                ...
                if res[0]:
                     ...
                     self.status_label.configure(text=f"✗ Lỗi: {res[1]}", text_color=Colors.DANGER)
            ...
            try:
                ...
                self.status_label.configure(text="Đang load model (chờ xíu)...", text_color=Colors.WARNING)
                self.update()
                ...
                self.status_label.configure(text="✓ Model Loaded (Ready)", text_color=Colors.SUCCESS)
                 ...
                 self.status_label.configure(text="✗ Thiếu thư viện llama-cpp-python", text_color=Colors.DANGER)
            except Exception as e:
                 self.status_label.configure(text=f"✗ Lỗi Load: {e}", text_color=Colors.DANGER)
