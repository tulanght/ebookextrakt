...
# description: Re-integrates content_structurer for intelligent PDF article splitting.
...
# Thêm import content_structurer
from ..core import pdf_parser, epub_parser, storage_handler, content_structurer
...
class MainWindow(ctk.CTk):
    ...
    # --- UI Creation (Không thay đổi) ---
    def _create_main_layout(self):
        self.grid_columnconfigure(0, weight=1)
        self.grid_rowconfigure(1, weight=1); self.grid_rowconfigure(2, weight=0); self.grid_rowconfigure(3, weight=0, minsize=120)
    ...
    def _create_welcome_screen(self):
        ...
        ctk.CTkLabel(self.welcome_frame, text="Vui lòng chọn một file Ebook để bắt đầu", font=("", 24)).pack(expand=True)
    ...
    def _create_results_view(self):
        ...
        self.results_frame_main.grid_columnconfigure(1, weight=1); self.results_frame_main.grid_rowconfigure(0, weight=1)
    ...
    # --- UI State and Logging (Không thay đổi) ---
    def _log(self, message):
        timestamp = time.strftime("%H:%M:%S"); formatted_message = f"[{timestamp}] {message}\n"
        self.log_textbox.configure(state="normal"); self.log_textbox.insert("end", formatted_message)
        self.log_textbox.see("end"); self.log_textbox.configure(state="disabled")
        self.update_idletasks()

    def show_welcome_screen(self):
        self.loading_frame.grid_forget(); self.results_frame_main.grid_forget()
        self.welcome_frame.grid(row=1, column=0, sticky="nsew")

    def show_loading_screen(self, filename):
        self.welcome_frame.grid_forget(); self.results_frame_main.grid_forget()
        self.loading_label.configure(text=f"Đang phân tích file:\n{filename}...")
        self.loading_frame.grid(row=1, column=0, sticky="nsew"); self.update_idletasks()
    
    def show_results_view(self):
        self.loading_frame.grid_forget(); self.welcome_frame.grid_forget()
        self.results_frame_main.grid(row=1, column=0, sticky="nsew")
        meta = self.current_results.get('metadata', {}); self.title_label.configure(text=meta.get('title', 'Không có tiêu đề'))
        ...
        if cover_path and Path(cover_path).exists():
            try:
                ...
                self.cover_label.configure(image=ctk_img, text=""); self.cover_label.image = ctk_img 
            except Exception as e:
                self._log(f"Lỗi khi load ảnh bìa: {e}"); self.cover_label.configure(image=None, text="[Lỗi ảnh bìa]")
        else:
            self.cover_label.configure(image=None, text="[Không có ảnh bìa]")
        self._display_content_tree()

    # --- Display Logic (Không thay đổi) ---
    def _clear_content_panel(self):
        for widget in self.content_panel.winfo_children(): widget.destroy()
    ...
    def _create_summary_label(self, master, node_data: Dict[str, Any]) -> ctk.CTkLabel:
        ...
        total_words = sum(len(str(data).split()) for c_type, data in content_to_summarize if c_type == 'text')
    ...
    # --- Worker Thread and File Handling (ĐÃ CẬP NHẬT) ---
    def _on_select_file_button_click(self):
        ...
        filename = Path(filepath).name; self._log(f"Đã chọn file: {filename}"); self._log("Bắt đầu quá trình phân tích...")
        self.current_filepath = filepath; self.select_button.configure(state="disabled"); self.save_button.configure(state="disabled")
        self.show_loading_screen(filename)
        self.progress_bar = ctk.CTkProgressBar(self, mode='indeterminate'); self.progress_bar.grid(row=2, column=0, padx=10, pady=5, sticky="ew")
        self.progress_bar.start()
        threading.Thread(target=self._worker_parse_file, args=(filepath, self.results_queue), daemon=True).start()
        self.after(100, self._check_results_queue)

    def _worker_parse_file(self, filepath, q):
        """Hàm worker được cập nhật để xử lý PDF qua content_structurer."""
        file_extension = Path(filepath).suffix.lower()
        results = {}

        if file_extension == ".pdf":
            # B1: Parser lấy dữ liệu thô
            raw_results = pdf_parser.parse_pdf(filepath)
            
            # B2: Structurer tinh chỉnh lại dữ liệu, xây dựng cấu trúc cây
            final_tree = []
            for chapter_node in raw_results.get('content', []):
                # Tách các bài viết con từ mỗi chương
                articles_in_chapter = content_structurer.structure_pdf_articles(chapter_node.get('content', []))
                
                children_nodes = []
                for article in articles_in_chapter:
                    # Bỏ qua các bài viết không có nội dung thực
                    if not article.get('subtitle') and not article.get('content'):
                        continue
                ...
                # Cập nhật node chương với các con đã được tách
                chapter_node['children'] = children_nodes
                # Xóa content cũ của chương cha để tránh trùng lặp
                chapter_node['content'] = [] 
                final_tree.append(chapter_node)

            # Cập nhật lại kết quả cuối cùng với cấu trúc cây đã hoàn thiện
            raw_results['content'] = final_tree
            results = raw_results

        elif file_extension == ".epub":
            # Parser EPUB đã trả về cấu trúc cây sẵn
    ...
    def _check_results_queue(self):
        try:
            ...
            if self.current_results and self.current_results.get('content'):
                self._log("Phân tích hoàn tất."); self.show_results_view(); self.save_button.configure(state="normal")
            else:
                error_content = (self.current_results.get('content') or [{}])[0].get('content', '[Không có chi tiết]')
                self._log(f"Lỗi: Không thể trích xuất nội dung. Chi tiết: {error_content}")
            self.select_button.configure(state="normal")
        except queue.Empty:
            self.after(100, self._check_results_queue)
    
    # --- Save Logic (Không thay đổi) ---
    def _on_save_button_click(self):
        target_dir = self.save_path_combobox.get()
        if not target_dir or not Path(target_dir).is_dir():
            messagebox.showerror("Lỗi Đường Dẫn", f"Đường dẫn không hợp lệ: {target_dir}"); return
        self._log(f"Bắt đầu lưu vào thư mục: {target_dir}...")
        self.save_button.configure(state="disabled"); self.select_button.configure(state="disabled")
        ...
        self.save_button.configure(state="normal"); self.select_button.configure(state="normal")
        if success:
            self._log(f"Lưu thành công vào: {message}")
            if messagebox.askyesno("Lưu Thành Công!", f"Đã lưu thành công vào:\n{message}\n\nBạn có muốn mở thư mục này không?"):
                try:
                    if os.name == 'nt': os.startfile(os.path.realpath(message))
                    elif os.name == 'posix': subprocess.run(['open', os.path.realpath(message)], check=True)
                except Exception as e:
